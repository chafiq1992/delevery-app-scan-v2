...
async def load_agent(session, username: str) -> Agent | None:
    result = await session.execute(
        ...
        .options(selectinload(Agent.drivers), selectinload(Agent.merchants))
    )
...
async def admin_list_agents():
    async for session in get_session():
        result = await session.execute(
            select(Agent).options(selectinload(Agent.drivers), selectinload(Agent.merchants))
        )
        return [
            AgentOut(
                username=a.username,
                drivers=[d.id for d in a.drivers],
                merchants=[m.id for m in a.merchants],
            )
